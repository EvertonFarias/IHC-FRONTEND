...
export class RegisterComponent {
  ...
  onSubmit() {
    ...
    this.http.post(`${environment.apiUrl}/auth/register`, payload)
      .subscribe({
        next: (response) => {
          this.showToast('success', 'Cadastro realizado com sucesso! Redirecionando...');
          setTimeout(() => {
  ...
  //  para verificar se um campo é inválido
  isFieldInvalid(fieldName: string): boolean {
    const field = this.registerForm.get(fieldName);
    return !!(field?.invalid && field?.touched);
  }

  // para formatar o nome de usuário (primeira letra maiúscula)
